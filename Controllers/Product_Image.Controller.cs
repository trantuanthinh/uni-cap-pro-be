...
namespace uni_cap_pro_be.Controllers
{
    ...
    ) : ControllerBase
    {
        ...
        private string _uploadFolderPath = Path.Combine(
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> CreateProduct_Image(
            [FromBody] Product_ImageDTO item,
            IFormFile file
        )
        {
            ...
            _uploadFolderPath = Path.Combine(
                Directory.GetCurrentDirectory(),
                $"Resources/{ownerId}/{productId}"
            );
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> PatchProduct_Image(
            Guid id,
            [FromBody] Product_ImageDTO item
        )
        {
